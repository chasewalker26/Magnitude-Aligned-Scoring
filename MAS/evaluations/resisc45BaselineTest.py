...
def full_like_resisc45_R101(input):
    mean = torch.mean(torch.tensor([0.368, 0.381, 0.344]))
    return torch.full_like(input, mean)

def full_like_resisc45_VIT16(input):
...
def run_and_save_tests(img_hw, transform, image_count, function_steps, batch_size, model, modified_model, it, model_name, device, dataset_path, dataset_name):
    ...
    if model_name == "R101":
        MAS_insertion_mean = MAS.MASMetric(model, img_hw * img_hw, 'ins', img_hw, substrate_fn = full_like_resisc45_R101)
        MAS_deletion_mean = MAS.MASMetric(model, img_hw * img_hw, 'del', img_hw, substrate_fn = full_like_resisc45_R101)
    elif model_name == "VIT16":
        MAS_insertion_mean = MAS.MASMetric(model, img_hw * img_hw, 'ins', img_hw, substrate_fn = full_like_resisc45_VIT16)
        MAS_deletion_mean = MAS.MASMetric(model, img_hw * img_hw, 'del', img_hw, substrate_fn = full_like_resisc45_VIT16)
    ...
    _, val_set = torch.utils.data.random_split(data, [train_split, val_split], generator = torch.Generator().manual_seed(42))
    ...
    for i, (img, target_class) in enumerate(val_loader):
        ...
        print(model_name + " baseline invariance test, " + dataset_name + " image " + str(counter + 1) + "/" + str(image_count))
        ...
        RISE_ins_mean_order.append(np.argsort(RISE_ins_mean_scores[counter])[::-1])
        RISE_del_mean_order.append(np.argsort(RISE_del_mean_scores[counter]))
        RISE_ins_del_mean_order.append(np.argsort(RISE_ins_del_mean_scores[counter])[::-1])
        MAS_ins_mean_order.append(np.argsort(MAS_ins_mean_scores[counter])[::-1])
        MAS_del_mean_order.append(np.argsort(MAS_del_mean_scores[counter]))
        MAS_ins_del_mean_order.append(np.argsort(MAS_ins_del_mean_scores[counter])[::-1])

        RISE_ins_rand_order.append(np.argsort(RISE_ins_rand_scores[counter])[::-1])
        RISE_del_rand_order.append(np.argsort(RISE_del_rand_scores[counter]))
        RISE_ins_del_rand_order.append(np.argsort(RISE_ins_del_rand_scores[counter])[::-1])
        MAS_ins_rand_order.append(np.argsort(MAS_ins_rand_scores[counter])[::-1])
        MAS_del_rand_order.append(np.argsort(MAS_del_rand_scores[counter]))
        MAS_ins_del_rand_order.append(np.argsort(MAS_ins_del_rand_scores[counter])[::-1])
